...
import { getSettings, addIcon, initTranslations } from './lib.js';

const Gsettings = getSettings(Me);
...
const SettingsBox = GObject.registerClass(class SettingsBox extends Gtk.Box {
  _init(setting) {
    ...
    if (Settings[setting].type === 's') widget = new SettingsEntry(setting);
    else if (Settings[setting].type === 'i') widget = new SettingsSpinButton(setting);
    else if (Settings[setting].type === 'b') widget = new SettingsSwitch(setting);
    else if (Settings[setting].type === 'e') widget = new SettingsCombo(setting);
...
const SettingsCombo = GObject.registerClass(class SettingsCombo extends Gtk.ComboBoxText {
  _init(setting) {
    ...
    Settings[setting].list.forEach(item => {
      ...
      if (item.id === Gsettings.get_enum(setting)) this.set_active(item.id);
    });
...
const SettingsEntry = GObject.registerClass(class SettingsEntry extends Gtk.Entry {
  _init(setting) {
    ...
    let placeholderText = Settings[setting].placeholder_text;

    if (placeholderText) {
      this.set_placeholder_text(placeholderText);
    }
...
const PrefsWidget = GObject.registerClass(class PrefsWidget extends Frame {
  _init() {
    ...
    for (let setting of Object.keys(Settings)) {
      ...
      if (Settings[setting].tab === 'i') this._indicatorPage.addSettingsBox(settingsBox);
      else if (Settings[setting].tab === 'p') this._playerControlsPage.addSettingsBox(settingsBox);
      else if (Settings[setting].tab === 'v') this._visibleWidgetsPage.addSettingsBox(settingsBox);
    }
  }
});
