...
import {
  MediaServer2,
  MediaServer2Player,
  MediaServer2Playlists,
  MediaServer2Tracklist,
  PithosRatings,
  RatingsExtension,
  Properties
} from './dbus.js';
import Settings from './settings.js';
import { parseMetadata } from './util.js';
...
export class MPRISPlayer {
  constructor(busName, owner) {
    ...
    MediaServer2(busName,
      proxy => {
        this._mediaServer = proxy;
        this._init2();
      });
    MediaServer2Player(busName,
      proxy => {
        this._mediaServerPlayer = proxy;
        this._init2();
      });
    MediaServer2Playlists(busName,
      proxy => {
        this._mediaServerPlaylists = proxy;
        this._init2();
      });
    MediaServer2Tracklist(busName,
      proxy => {
        this._mediaServerTracklist = proxy;
        this._init2();
      });
    PithosRatings(busName,
      proxy => {
        this._pithosRatings = proxy;
        this._init2();
      });
    RatingsExtension(busName,
      proxy => {
    ...
    this.connect('update-player-state', (player, state) => {
      // global.log(JSON.stringify(state));
      this.state.update(state);
      if (state.status) this._onStatusChange(state);
  ...
  _init3() {
    if (Settings.MINOR_VERSION > 19) {
      ...
      this._signalsId.push(
        this._settings.connect('changed::' + Settings.MEDIAPLAYER_HIDE_STOCK_MPRIS_KEY, (settings, key) => {
    ...
    // showVolume setting
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_VOLUME_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_POSITION_KEY, (settings, key) => {
    ...
    // showRating setting
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_RATING_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_PLAY_STATUS_ICON_KEY, (settings, key) => {
        this.emit('update-player-state', new PlayerState({ showPlayStatusIcon: settings.get_boolean(key) }));
      })
    );
    // showTracklistRating setting
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_TRACKLIST_RATING_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_STOP_BUTTON_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_LOOP_STATUS_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_BUTTON_ICON_STYLE_KEY, (settings, key) => {
        this.emit('update-player-state', new PlayerState({ buttonIconStyle: settings.get_enum(key) }));
      })
    );
    // showPlaylists setting
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_PLAYLISTS_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_PLAYLIST_TITLE_KEY, (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect('changed::' + Settings.MEDIAPLAYER_TRACKLIST_KEY, (settings, key) => {
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal('TrackListReplaced', (proxy, sender, [trackIds, currentTrackId]) => {
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal('TrackAdded', (proxy, sender, [trackMetadata, afterTrackId]) => {
        let insertIndex = -1;
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal('TrackRemoved', (proxy, sender, [trackId]) => {
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal('TrackMetadataChanged', (proxy, sender, [trackId, updatedMetadata]) => {
        this.emit('update-player-state', new PlayerState({ updatedMetadata: updatedMetadata }));
      })
    );

    this._playlistsId = this._mediaServerPlaylists.connectSignal('PlaylistChanged', (proxy, sender, [updatedPlaylist]) => {
    ...
    this._seekedId = this._mediaServerPlayer.connectSignal('Seeked', (proxy, sender, [value]) => {
      if (value > 0) {
  ...
  populate() {
    ...
    if (newState.playlistCount > 0 && newState.playlistTitle) {
      this._playlistTimeOutId = Mainloop.timeout_add_seconds(1, () => {
        this._playlistTimeOutId = 0;
        this._getPlaylists(this.state.orderings);
        return false;
      });
    ...
    if (newState.hasTrackList && !isDummyTracklist) {
      this._tracklistTimeOutId = Mainloop.timeout_add_seconds(1, () => {
        this._tracklistTimeOutId = 0;
        this._getTracklist();
        return false;
      });
  ...
  get showLoopStatus() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_LOOP_STATUS_KEY) &&
      this.shouldShowLoopStatus && !this.noLoopStatusSupport;
  }

  get showStopButton() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_STOP_BUTTON_KEY) &&
      !this.playerIsBroken;
  }
  ...
  get showVolume() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_VOLUME_KEY) &&
      !this.playerIsBroken;
  }
  ...
  get showRating() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_RATING_KEY) &&
      !this.playerIsBroken;
  }

  get showPlaylist() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_PLAYLISTS_KEY) &&
      !this.playerIsBroken && !this.isClementine;
  }

  get showPlaylistTitle() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_PLAYLIST_TITLE_KEY) &&
      !this.playerIsBroken && !this.isClementine;
  }

  get showTracklist() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_TRACKLIST_KEY) &&
      !this.playerIsBroken && !this.isClementine;
  }

  get showTracklistRating() {
    return this._settings.get_boolean(Settings.MEDIAPLAYER_TRACKLIST_RATING_KEY) &&
      !this.playerIsBroken;
  }
  ...
  _getPlaylists(orderings) {
    ...
    this._mediaServerPlaylists.GetPlaylistsRemote(0, 100, ordering, false, ([playlists]) => {
  ...
  _getTracklist() {
    ...
    if (this._trackIds.length === 0) {
      ...
      this._mediaServerTracklist.GetTracksMetadataRemote(this._trackIds, ([trackListMetaData]) => {
  ...
  destroy() {
    ...
    // FIXME: i have no clue what it does but for in is not a good thing
    for (let id in this._tracklistSignalsId) {
      if (id) this._mediaServerTracklist.disconnectSignal(this._tracklistSignalsId[id]);
    }

    for (let signal of this._signalsId) this._settings.disconnect(signal);
  }

  toString() {
    return '<object MPRISPlayer(%s)>'.format(this.identity);
  }
}
