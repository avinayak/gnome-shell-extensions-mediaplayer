...
export default class PlayerUI extends Widget.PlayerMenu {
  constructor(player) {
    ...
    this.playlistTitle.connect('activate', this.player.raise.bind(this.player));
    ...
    this.trackCover.connect('activate', this.player.raise.bind(this.player));
    this.addMenuItem(this.trackCover);
    this.trackCover.hide();

    this.trackRatings = new Widget.TrackRating(this.player);
    this.trackRatings.connect('activate', this.player.raise.bind(this.player));
    this.addMenuItem(this.trackRatings);
    this.trackRatings.hide();

    this.info = new Widget.Info();
    this.info.connect('activate', this.player.raise.bind(this.player));
    this.addMenuItem(this.info);

    this.trackControls = new Widget.PlayerButtons();
    this.trackControls.connect('activate', this.player.raise.bind(this.player));
    ...
    this.shuffleLoopStatus.connect('activate', this.player.raise.bind(this.player));
    ...
    this.position.connect('activate', this.player.raise.bind(this.player));
    this.position.sliderConnect('value-changed', item => this.player.seek(item._value));
    this.addMenuItem(this.position);
    this.position.hide();

    this.volume = new Widget.SliderItem('audio-volume-high-symbolic');
    this.volume.connect('activate', this.player.raise.bind(this.player));
    this.volume.sliderConnect('value-changed', item => {
  ...
  update(player, newState) {
    if (newState.desktopEntry !== null) {
      this.icon.icon_name = getPlayerSymbolicIcon(this.state.desktopEntry);
    }
    ...
    if (newState.hideStockMpris !== null) {
      if (this.stockMpris) {
        if (newState.hideStockMpris) {
          this.stockMpris._shouldShow = function() {
            return false;
          };
    ...
    if (newState.volume !== null) {
      ...
      let value = this.state.volume; let volumeIcon;
      if (value === 0) {
