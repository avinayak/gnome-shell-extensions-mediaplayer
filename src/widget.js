...
import { setCoverIconAsync, animateChange, parseMetadata } from './util.js';
import { RhythmboxRatings } from './dbus.js';


export class SubMenu extends PopupMenu.PopupMenuBase {
...
export class BaseContainer extends PopupMenu.PopupBaseMenuItem {
  constructor(parms) {
    super(parms);
    this._hidden = false;
    ...
    this.actor.add_style_pseudo_class = () => null;
  }
...
export class CenteredBaseContainer extends BaseContainer {
  constructor(parms) {
    super(parms);
...
export class PlayerButtons extends CenteredBaseContainer {
  constructor() {
    super({ hover: false });
...
export class ShuffleLoopStatus extends BaseContainer {
  constructor(player) {
    super({ hover: false });
    this._player = player;
...
export class PlaylistTitle extends CenteredBaseContainer {
  constructor() {
...
export class PlayerButton {
  constructor(icon, callback) {
    ...
    this.actor.connect('notify::hover',
      button => this.actor.opacity = button.hover ? 255 : 204);
  }
...
export class SliderItem extends CenteredBaseContainer {
  constructor(icon) {
    super({ hover: false });
...
export let TrackCover = class TrackCover extends CenteredBaseContainer {
  constructor(icon) {
    super({ hover: false, style_class: 'no-padding-bottom' });
    this.icon = icon;
...
export let Info = class SecondaryInfo extends CenteredBaseContainer {
  constructor() {
    super({ hover: false, style_class: 'no-padding-bottom' });
    this._animateChange = animateChange;
...
export class TrackRating extends CenteredBaseContainer {
      ...
      if (this._player._ratingsExtension) {
        ...
        if (this._isNuvolaPlayer) {
          ...
          if (supported[this._player.busName]) {
            this._rhythmbox3Proxy = new RhythmboxRatings(this._player.busName);
  ...
  _rate(value) {
    ...
    this._value = value;
    for (let i = 0; i < 5; i++) {
      let iconName = i < this._value ? 'starred-symbolic' : 'non-starred-symbolic';
      if (this.animating) {
        this._starButton[i].child.icon_name = iconName;
      } else {
        let starChild = this._starButton[i].child;
        Mainloop.timeout_add(50 * i, () => {
          this._animateChange(starChild, 'icon_name', iconName);
          return false;
        });
      }
    }
...
export class ListSubMenu extends PopupMenu.PopupSubMenuMenuItem {
  constructor(label) {
    super(label, false);
...
export class TrackList extends ListSubMenu {
...
export class Playlists extends ListSubMenu {
  constructor(label, player) {
    super(label);
    this.player = player;
  }
...
export class PlaylistItem extends PopupMenu.PopupBaseMenuItem {
...
export class TracklistItem extends PopupMenu.PopupBaseMenuItem {
  constructor(metadata, player) {
    ...
    this._setCoverIconAsync = setCoverIconAsync;
    this._animateChange = animateChange;
    this._rating = null;
    this._coverIcon = new St.Icon({ style_class: 'small-cover-icon' });
    let _iconBox = new St.BoxLayout({ height: 48, width: 48 });
    _iconBox.add(this._coverIcon, { y_fill: false, y_align: St.Align.MIDDLE });
    this._artistLabel = new St.Label({ style_class: 'track-info-artist' });
    this._titleLabel = new St.Label({ style_class: 'track-info-title' });
    this._albumLabel = new St.Label({ style_class: 'track-info-album' });
    ...
    this.actor.add(_iconBox, { y_fill: false, y_align: St.Align.MIDDLE });
  ...
  _buildStars(value) {
    ...
    this._starIcon = [];
    for (let i = 0; i < 5; i++) {
      let iconName = i < value ? 'starred-symbolic' : 'non-starred-symbolic';
      ...
      Mainloop.timeout_add(50 * i, () => {
        this._animateChange(starIcon, 'icon_name', iconName);
        return false;
      });
    }
    this._rating = value;
